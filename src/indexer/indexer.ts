...
export async function connectToChain(): Promise<ApiPromise> {
  ...
  // Update config with live values
  ...
  try {
    ...
    // Try to get ledger version from system properties
    const props = await api.rpc.system.properties();
    const propsJson = props.toJSON() as any;
    if (propsJson?.ledgerVersion) config.node.ledgerVersion = propsJson.ledgerVersion;
  } catch {}
...
export async function fetchEpochInfo(apiInstance: ApiPromise): Promise<void> {
  try {
    // Use raw HTTP POST since sidechain methods may not be in metadata
